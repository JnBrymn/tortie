...
┌─ TITLEBAR [h:38] bg --bg-sidebar, 1px bottom border drawn transparent ───────────┐
│ ○○○ ·76px· [tab][tab][tab] [+]                 (spacer)               [🔔 3] ·12·│
│  ·          ·8╭─────────────────────────────────────┬───────────────╮8·          │
│AB· view     · │ session tab strip (default)         │ editor tabs   │ · SESSIONS │ ← HEADER BAND
│48· header   · │ or identity strip ("right")         │               │ · toolbar  │   [h:36], its hairline
│  ·          · ╞═════════════════════════════════════╪═══════════════╡ ·          │   INSIDE the work only
│  · SIDEBAR  · │ CENTER (terminal)                   │ EDITOR        │ · SESSION  │
│  · w:280    · │ flex:1, min-w:640                   │ (when open)   │ · LIST     │
│  · min:220  · │ (min-w:560 when right list visible) │ w:45% of ctr  │ · ("right" │
│  · max:400  · │ xterm.js  bg --bg-canvas            │ min:480       │ · only)    │
│  · one view · │ padding 8px 12px                    │ Monaco        │ · w:200    │
│  · SCM/Expl · │                                     │ bg --bg-canvas│ · 160–320  │
│  ·          ·8╰─────────────────────────────────────┴───────────────╯8·          │
└──────────────────── 8px of --bg-sidebar under the work ──────────────────────────┘
```

`╭ ╮ ╰ ╯` is THE WORK'S OUTLINE (Phase 284): `.work-area::after`, 1px `--border-strong`, radius `--r-frame` (14px), drawn 1px OUTSIDE the work's box with `pointer-events: none` at `--z-editor-overlay + 1`, and it is the only complete region-sized outline in the window. `·` is a boundary whose 1px border is still there and is `transparent`. `8` is `--frame-gap`, a gutter of `--bg-sidebar`.

- **HEADER BAND** (round-1): every region except the activity bar puts its header in one 36px band at `y = 38…74`: sidebar view header (S3), terminal-region header (session tab strip by default; identity strip in "right" orientation — S4), editor tabs row (S5, now 36px), and the right session list's toolbar (S4). All `[h:36]`, bg `--bg-sidebar`, same y in every region, no offsets. **The 1px `--border` bottom hairline is drawn INSIDE THE WORK ONLY (Phase 284)**: pixel-continuous from the work's left edge to its right edge, meeting the outline at both ends in a T, with exactly one sanctioned interruption, the gap under the ACTIVE session tab, where `--bg-canvas` runs through to the terminal (standard tab affordance, S4). The view header, the list toolbar, the project rail's band and the activity bar's row form keep their 1px bottom border and draw it `transparent`. The band is the first 36px inside the curve in BOTH orientations, and `.stab-list > .stab:first-child` takes `calc(var(--r-frame) - var(--frame-edge))` on its top-left corner so its accent inset follows the curve. Acceptance: S12.9. *Reversed by the operator on 2026-09-17: until then this bullet required ONE shared hairline from the sidebar's left edge to the window's right edge, with the vertical dividers crossing it.*
- **Activity bar**: w:48 fixed, full height below the titlebar, bg `--bg-sidebar`, 1px right border drawn `transparent` (Phase 284; a `--border` hairline until the operator's reversal of 2026-09-17); never hides (spec S3). The band does not cross it.
- **The work's frame (Phase 284)**: `.work-area` on `--bg-canvas`, margins `0 8px 8px 0` plus `8px` on the left only while the sidebar is drawn, all from `--frame-gap`; `.shell-body` takes `--bg-sidebar` so the gutters and the four wedges outside the curve read as the surround, while `.shell` stays `--bg-canvas` for the window's pre-paint fill. The CLIP is on the frame's CHILDREN (`overflow: clip`, never `hidden`, which is a scroll container a focused xterm textarea would scroll), at the inner radius of 13px, and never on `.work-area` itself, which would clip its own outline. The scroll lane, every split, the editor in split, overlay and fill modes with its scrim, and the split drop zone all sit inside it. Two things that sit in its bottom corners keep their shape there (fix round): the focused split's box follows the curve on a bottom corner it shares with the frame, found by structure and never by a stamp on the DOM, and the editor's body lifts every root it draws by 8px so Monaco's slider, the diff's, the preview's and the report's end level with the terminal lane rather than under the arc. Focus mode turns the frame off (no gutter, no radius, no line), and Catch Me Up does not draw the work at all. The 8 and the 14 are `FRAME_GAP` and `FRAME_RADIUS` in `chrome-geometry.ts` by value, and `workAreaWidth` subtracts the gutters, so every visible session is resized ONCE at the first paint after the upgrade and never during a flight (S12.7 stands).
- **Right session list** (only when orientation = "right", S4): w:200 persisted, drag 160–320 by the handle in the gutter on its left (its own left border is `transparent` since Phase 284); while visible the center's min-w drops to 560 (xterm refits).
- Region boundaries (Phase 284; "Region dividers: 1px `--border`" until the reversal of 2026-09-17): the work's outline and its gutters, and no divider. Drag handles: a 5px invisible hit area centred in the 7px of gutter the outline leaves free, 1px clear of the line; hover and drag show it at `--border-strong`, `:focus-visible` at `--accent`; cursor `col-resize`. Both are `role="separator"` tab stops that ← → move. Seams INSIDE the work (split dividers, the editor's left border) stay 1px `--border`. Sidebar and editor widths persist per project; right-list width and orientation persist app-wide.
...
- Tab width 96px to 200px, and the name truncates at its end with one CSS ellipsis. **The 96px floor is measured, not chosen** (Phase 189): 46px of name, which is the first four characters of every one of the operator's own project names with 4.1px of headroom, plus the 50px of chrome that cannot shrink. A tab carrying a mark has a bigger floor by exactly what the mark occupies. This line said "truncates middle (keep suffix)" and "≥10 tabs" until Phase 189, and both were wrong: the JS middle-truncation cut at a character count that could not see the tab, so the CSS ellipsis clipped its result a second time and the active tab drew `extract-agen…en…`; and the trigger is width rather than a count, since twelve tabs fit at 1512px and eight do not at 960px. **The row itself scrolls** (`.ptab-list`, horizontal, no visible scrollbar), and the chevron appears at the row's end only while it overflows. **⌘1–⌘8 are positions and ⌘9 is the LAST tab however many are open** (Phase 12.12 item 3, the browser convention): "the ninth" left the tail of a long strip unreachable. Both directions of the rule come from one module, `src/renderer/app/project-shortcuts.ts` — what the keystroke does and what a tab claims below cannot disagree.
...
- **Tab drag-reorder (round 2, user ref media_cWSQ48lD7j):** press + 4px pointer travel lifts the tab — ghost at 90% opacity, `--shadow-2`, follows the pointer on x only (y clamped to the bar). Neighbors do NOT reflow during the drag; instead a 2px `--accent` vertical insertion indicator `[h:20, r:1px]` marks the gap the tab will land in. Drop settles in 160ms `--ease-out`; Esc cancels (tab snaps home, no motion). Order is app-wide persisted state; ⌘1–⌘9, ⌃Tab cycling, and the ≥10-tab overflow menu all follow the visual order. Dragging past either end auto-scrolls the strip, and since Phase 189 the project row does that too, through `edgeAutoScroll` in src/renderer/app/split/pointer-drag.ts. `+` and 🔔 are never drop targets; a drag never leaves the titlebar row (project tabs cannot be dropped into the terminal).
...
Full height below the titlebar, bg `--bg-sidebar`, 1px right border drawn `transparent` (Phase 284; a `--border` hairline until the operator's reversal of 2026-09-17). No horizontal hairline crosses it at the band's y.
...
- States: active — icon `--text-primary` on a `--bg-active` CHIP, 36px inside the 48px cell on `--r-md` (a 28px chip in the 36px row form), and no marker bar (Phase 284; a 2px `--accent` inset bar on the item's LEFT edge until the operator's reversal of 2026-09-17); inactive — `--text-muted`; hover — `--text-secondary` on a `--bg-raised` chip (the fill is Phase 150's, the chip is Phase 284's); `:focus-visible` — `--focus-ring` inset. The hit area is the whole cell. The fill is on the button itself with `background-clip: content-box`, and the hover rule sets `background-color`, never the `background` shorthand, which resets the clip and squares the chip on hover only.
...
Padding 0 12px, bg `--bg-sidebar`, 1px bottom border drawn `transparent` (Phase 284: the band's hairline is inside the work only, S1; Source Control's `.branch-header` is that view's header and is quiet in the same way). Content per view (below). Right accessories are icon-buttons 20×20, codicon 16px, `--text-secondary`, hover `--bg-raised` r-sm.
...
**CHANGES section wrapper (round 2):** a "▾ CHANGES n" sticky header `[h:24]` (n = total files across Merge/Staged/Changes/Untracked; styled as every section header — 11px/600 uppercase `--text-muted`, count 11px `--text-muted`) now sits above the commit box; the box + file groups are its body (collapsing hides both), and the header doubles as the section's drag handle (S3 block). **Group rows and SCM file rows keep their round-0 geometry; the file rows gain round-2 multi-selection (S3D). The commit box no longer does: Phase 149 moved it onto the shared side view scale, and the clause below is the current one.** Commit box **(Phase 149)**: textarea auto-grow 1 to 5 lines, `--view-field-text` (12px) `--font-ui` on `--lh-base` (20px) leading, min-height `--field-h` (28px), bg `--bg-surface`, border 1px `--border-strong`, r-sm, padding `--space-2` `--space-4` (4px 8px). It reads the SAME four names every other side view reads, so the operator's rule that Explorer, Search, Source Control and Context share one type scale is stated in `tokens.css` §1.8b and never restated here as a number. The 28px is 20px of leading plus 4px of padding above and below, so one line of a commit message rests at the scale's own height. Five lines is still the ceiling, and the autosize in `ScmSection.tsx` reads the element's own computed leading and padding rather than repeating either figure; [Commit] primary full-width `[h:28]` ("Committing…" + 12px spinner while running); ⌘↩ commits staged; nothing staged → "Stage all & commit". Group rows `[h:24]`: label 11px/600 `--text-secondary` + count 11px `--text-muted`; order Merge (when present), Staged, Changes, Untracked. SCM file row `[h:24]`, padding-left 20px: status letter mono 11px/600 in git color (M/A/U/D + strikethrough/R/`!` conflict) · filename 12px `--text-primary` · dir path 11px `--text-muted` truncated left; hover `--bg-raised` + stage ＋ / discard ↩ / unstage － icons 16px right; discard confirms before it runs; click → diff-vs-HEAD (S5); untracked → plain file. **Rows are multi-selectable and every verb follows the selection — see S3D.**
...
```
┌ HEADER BAND [h:36] = TAB STRIP · bg --bg-sidebar · the work's band hairline ───┐
...
- **Right dock**: w:200 persisted (drag 160–320 by the handle in the 8px gutter on its left; the dock's own left border and its toolbar's bottom border are `transparent` since Phase 284), bg `--bg-sidebar`, own scroll, full height under the band.
...
- **Row `[h:24]`**, padding 0 8px, r `--r-md` inset 4px: agent icon 16 · name 13px (needs-input 500) · `⎇wt` chip mono 10px on `--bg-raised` `[h:16]` (when in a worktree) · spacer · status dot 8 · × 16 on hover (End-session confirm). Selected: `--bg-active` fill + `outline: 1px solid var(--border-active)` at `outline-offset: -1px` on the row's `--r-md`, and NO marker bar (Phase 284; a 2px `--accent` left inset until the operator's reversal of 2026-09-17). It is an `outline` and not an inset `box-shadow`, because the row's selector outranks the global `:focus-visible` ring and a shadow here would silently take a focused row's ring. The collapsed 48px rail and the project rail are full-bleed rows and take the fill alone. Hover: `--bg-raised`. Saved: codicon `history` 12 after the dot. Age + status text in tooltip (density mirrors VS Code's 22px terminal list, on our 4px grid). List is focusable: ↑↓ ↩, F2, context menu.
...
- Terminal focus signal: the band's bottom hairline turns `--accent` (1px) when the terminal has focus (round-0 rule, carried to the band, and kept whole by Phase 284: the band stays on `--bg-sidebar` with its rule at rest so there is a line to turn, which is where the app departs from the study). With splits, the signal means "the surface's focused split has keyboard focus"; the per-split ring (S4A) says which one.
...
- **Focus**: exactly one focused split per surface. Focused: header name `--text-primary`, plus ONE 1px `--accent-soft` box around the WHOLE split, header included, drawn as a `::after` overlay with `pointer-events: none` (static — no animation). Phase 40 replaced the 2px left inset and the hard inset ring this line still described until Phase 284 corrected it. It is the precedent the work's outline follows, and the one other region-sized box the window may draw (S12.9). Unfocused headers: `--text-secondary`. Click anywhere in a split focuses it.
...
- Tabs row `[h:36]` (round-0 32px is gone): in split mode its bottom hairline IS the work's band hairline (S1); in overlay mode the row keeps the same 36px height inside the floating panel.
...
- **A block that is not prose gets what its CONTENT needs, up to twice the measure (Phase 248, resized by Phase 252).** The 68ch measure is for PROSE and it does not change; the two children that already carry `overflow-x: auto` — a table's `.md-table-scroll` and a code fence — break out of it and take their own `max-content`, floored at the prose column and capped at `min(136ch, 100cqi / var(--zoom-editor, 1) - 2 × --space-8)`. Phase 248 gave them the cap unconditionally, and the operator's screenshots of 2026-09-10 showed why that reads as a defect: an ASCII diagram needing ~75ch drawn as a near-2,000px box, most of it empty inside its own border. A fence never re-wraps — `max-content` against `white-space: pre` measures its longest line — and a block wider than the cap keeps the scroller. **The centring is `left: 50%` + `translate: -50%`**, because the old negative margin was computed from the same expression as the width and a content-measured width cannot be read back into a calc(); the pair centres any used width on the column's axis, measured at 49 boxes within 1px across three panes, both bases and zoom stops above and below 1, with the losing grid candidate recorded in the stylesheet comment (it stops sibling margins collapsing and grew the fixture's document 84px). **`100cqi` is the PANE**, which is why `.md-scroll` carries `container-type: inline-size` and nothing else on this surface does: `100vw` is the window, and in a resizable split beside a terminal it drew a 1,900px block in a 620px pane and scrolled the document sideways at every width — the one thing this surface promises never happens. **The cap is spelled in the measure's own unit and registered with `@property`** so it computes once, in the prose font: substituted as text it would resolve against `--font-editor` inside `pre` and cap the fence at 981px against the table's 1,114px. Before this, a table's box was 509px at a 620px pane, a 1,000px pane and a 1,381px pane alike, and 557 of this repository's 1,900 real tables lost a column; at the cap 16 do, and they keep the scroller they have in a box more than twice as wide. Growth stops at a pane of 1,152px. Measured in research 112. **The break-out reaches a DIRECT child of `.md-content` and nothing deeper**, because both halves of it resolve against the block's containing block: under a bullet that is the list item, inset 20px a level, so a table under a sub-bullet drew 11px past the pane and scrolled the document sideways (research 112 §10.1). A nested block keeps the scroller it always had, which costs 3 of this repository's 1,913 tables and 14 of its 817 fences the room, three of those fences excluded by a blockquote rather than by an indent. **The pane term is divided by the editor zoom for a reason**, because `zoom.css` scales `.md-content` and multiplies every used length under it, including the one term measured outside that subtree: two presses of ⌘+ drew a 1,392px block in a 1,381px pane and 2,227px at 200% (§10.2). **A table narrower than the box it was given is centred in it** — 226 of the 1,900 already fitted the measure and would otherwise be drawn 227px left of every paragraph around them, and CSS cannot ask how wide a table wants to be before giving it the room (§10.3). `npm run conformance:wideblocks` is what keeps every clause of it true. **The one affordance change is a number**: the table box's scrollbar thumb is `--text-muted` rather than the shared `--border-strong`, which read 1.594:1 on the canvas against WCAG 1.4.11's 3:1, and its hover lifts rather than dropping to `--text-disabled`.
...
- **A preference only a diff can honour lives in the diff, not in the actions row.** Phase 185 gave the diff surface its own 30px control row at its head (`DiffControls`) for how a change is drawn: the inline highlighting and the changed-row wash. Both are persisted app-wide like the two above, so persistence is not what decides the placement — reach is. The two in the actions row each reach past this surface: `minimapApplies` in `EditorPanel.tsx` keeps the minimap out of diff and image chrome altogether because it belongs to the text modes, and one `sideBySide` prop drives the image comparison as well as this diff (S5D). Neither of the two new answers can be honoured anywhere but Pierre's diff, so they sit beside the thing they change. The row draws only when a diff is actually on screen, never over the skeleton or the "No changes" state. Put the next diff-only preference there, not beside the minimap.
...
**Chrome**: a transparency checkerboard drawn from `--bg-canvas` + `--bg-raised` on the IMAGE element (so it marks the picture's own bounds and travels with pan/zoom while its squares stay 16px on screen — a bright checker would be the loudest object in a dark world, which belongs to the amber needs-input dot). Under it a 26px footer: `1920 × 1080 · 412 KB · PNG` on the left, the zoom readout and a `Fit | 1:1` pair on the right. The zoom controls live HERE, not in the tab strip's actions row, because that row holds preferences that apply to whatever the tab is showing (minimap, side-by-side) while zoom is ephemeral state belonging to one picture and needs its readout beside it. A preference that only a diff can honour goes in the diff's own control row instead — see S5C.
...
9. **The one outline and the quiet surround** (Phase 284; until the operator's reversal of 2026-09-17 this item was "one 36px band, one hairline" across the whole window). Three things, read off rectangles, computed styles and one painted pixel per corner by `npm run probe:p284`: (a) exactly ONE complete region-sized outline exists in the window, `.work-area::after`, 1px `--border-strong` on 14px, with 8px gutters to both sidebars and 8px of right and bottom inset; with a split on screen exactly one more box is sanctioned, Phase 40's focused-split box; (b) INSIDE it the band is still one 36px band with one unbroken 1px hairline at one y across center and editor, whose only sanctioned break is the gap under the active session tab, and which still turns `--accent` while a terminal has the keyboard; (c) the surround draws NO hairline facing the work: the titlebar's bottom, the activity bar's right (and its row form's bottom), the sidebar's right, the view header's bottom, Source Control's branch header's bottom, the session list's left, its toolbar's bottom and the project rail band's bottom each read alpha 0 and STILL 1px wide, so the 36px heights align to the pixel at every window width. The outline's contrast against both grounds is no lower than the 1.297:1 hairline it replaces, on both bases and at a turned hue.
...
15. **Splits**: the focus box (S4A) tracks the focused split; ⌘⌥ arrows navigate geometrically with the ↓/↑ edge fallthrough — verify unsplit surfaces behave key-for-key as round 1; pop-out works by drag AND by menu in BOTH orientations; layout, ratios, and focused split survive relaunch; banners and Ready-to-restore render split-scoped; no rendered string contains "pane" (extend the item-6 vocabulary grep).
...
- **Terminal font**. ~~size stepper~~ **The SIZE half was WITHDRAWN in Phase 12.11, not deferred again. The FAMILY half LANDED in Phase 78.** The size half is answered by per-region zoom (S14): ⌘+ / ⌘- change the terminal's font size for real and push the new geometry to tmux, and a Settings size field beside it would be a second answer to the same question fighting the first. The `--font-terminal` token ships and remains the family lever; nothing in the UI promises a control that does not exist. Phase 78 built the family picker this line sanctioned, and it sets that token. It lives in Settings, Appearance, in a Font group under Contrast, and it offers three presets. They are System, JetBrains Mono and Source Code Pro. System is the default and writes no override, so an install that never opens the section renders exactly what it rendered before. A bundled preset writes `--font-terminal` and the new `--font-editor`, which carries the same face into Monaco, the Pierre diffs and the code in the markdown preview. The sidebar and the rest of the chrome keep their own tokens and do not move: the Explorer tree and the body read `--font-ui` (`FileTree.tsx` hands the tree `var(--font-ui)`, `globals.css` sets it on `body`), while the SCM sidebar, search and quick open read `--font-mono`. An earlier draft of this sentence named `--font-mono` for the whole sidebar, which was never what the tree drew (Phase 197 item 15). Zoom stays a multiplier over whatever base size the chosen face implies (DESIGN.md §10.1), and Settings still has no size field.

### Appearance
Five groups, in this order: Scheme, Highlight, Frame, Contrast, Font. Every control writes one field and the live window is the preview; there is no Save.

- **Scheme** (Phase 213), one segmented control, three names: Light, Dark, Match the Mac. Dark is shipped and byte identical to before the control existed. Light is the paper base of DESIGN.md section 1.1b, and the TERMINAL GOES LIGHT WITH IT, never a light frame around a dark terminal. Match the Mac follows the system appearance live. The chosen segment is filled `--accent` with `--on-accent` lettering; the words live on the hover titles ("Paper with dark text", "Graphite with light text, the shipped look", "Follows the Mac and changes with it") and the caption is one line. A change of scheme crossfades the whole window over `--dur-panel`; reduced motion switches in one frame. On the light base the Shade row is NOT SHOWN (Phase 214), because paper carries one shade and a control that cannot move is not drawn; its refusal line goes with it, since there is nothing to refuse. Depth stays and narrows only, and its refusal line says whether the dots or the accent stopped it; `npm run conformance:hue` pins that region and rule 31 asserts that no base offers a control with a single stop.
- **Highlight**, one dropdown, four schemes, Blue shipped (Phase 62).
- **Frame** (Phase 207, rebuilt in Phase 210), three controls and a strip.
  - **Color**, a row of EIGHT named starting colours, each drawn as the frame it produces at the shade and depth in effect: a 28x20 chip of the five grounds in ramp order, the name 11px `--text-muted` under it, `--accent` border and `--text-primary` name on the chosen one. Graphite is the shipped ramp and the default. **NO DEGREE IS DRAWN ON THE RESTING FACE**: it survives in the persisted setting and on the hover title, at the operator's word of 2026-09-03 that the degree told him nothing about what he would see. A hue none of the eight carries keeps a swatch of its own at the head of the row rather than being snapped onto a neighbour.
  - **Shade**, a seven stop slider, where the ramp sits, from near black upward. **Depth**, a seven stop slider, how far the panels and hairlines stand apart. Both default to the shipped ramp and write nothing there. Each is drawn only where it can move: on paper the offered region is one shade row, so Shade is absent there and the setting underneath is untouched, which is what brings the shade back when Dark is chosen again.
  - **A slider STOPS at the last stop that keeps every contrast floor** and says why in one short line, reserved in layout whether or not it speaks (the Phase 174.1 rule). It does not shrink its own track and it does not accept the move and clamp it. Where the other control can rescue the move the line says that instead: "Darker needs more depth."
  - **The strip**, the five grounds side by side with the hairline drawn between them, at the frame all three controls produce. It is the readout, and the reason the hairline is in it is that the depth is otherwise invisible.
  - The colour arithmetic and the floors are DESIGN.md section 1.1; `npm run conformance:hue` is what keeps that paragraph true and `npm run probe:p210` is the app run.
- **Contrast**, one dropdown, three steps, Normal shipped (Phase 62), plus the recorded limits on the same card.
- **Font**, see the Terminal font line above.
