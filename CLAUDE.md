...
| `src/main/arch/facts/`, `src/main/arch/tree-facts.ts`, `src/main/arch/fact-parser.ts`, `src/main/symbols/{queries,calls,wrappers,extract,worker,pool}.ts`, the `arch_fact*` half of `src/main/arch/db.ts`, the fact types in `src/shared/arch.ts` | `conformance:facts` | ~40 s, no Electron, no git, reads nothing under his home | The fact base's reader pinned over eight committed fixtures byte for byte, every `[ipc.invoke.channels]` entry of the contract baseline read as `IPC serves` with the wrapper pass on (229 of 229, 0 extras, the count read from the baseline), the wrapper pass's three clauses, the dedupe, the vendor filter, the domain's purity, the closed kind sets and the boundary split, and 22 ablations that must each go red. `probe:p257` is the corpus run (Phase 257) |
