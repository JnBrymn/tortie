...
export const AGENT_REGISTRY: readonly AgentRegistryEntry[] = [
  ...
  },
  {
    ...
    resume: {
      ...
      sessionStore:
        '~/.omp/agent/sessions/<dir key>/<ISO ts, :.→->_<sessionId>.jsonl. The dir key is NOT pi’s flat wrap: omp buckets the realpathed cwd (sanitizeOmpCwd in harvest/stores.ts) as -<rel> under HOME, -tmp-<rel> under the realpathed tmpdir, and only a cwd outside both keeps pi’s --<abs>-- wrap.',
      notes:
        ...
        'CWD-keyed store dir is a pure function of the cwd (sanitizeOmpCwd, the scoped scheme above, not pi’s sanitizePiCwd), so harvest needs no scan: watch the one directory, match the filename uuid, confirm on line-2 cwd. ' +
    ...
    multilineKey: {
      ...
      verified: true,
      notes:
        'MEASURED 2026-08-30 on omp 18.0.11 (Phase 169): one LF byte sent to the prompt with text on either side of it left the two texts on two editor lines and started no turn. The pi lineage inference held.'
    },
    imageDrop: {
      strategy: 'paste-path',
      insert: 'paste',
      verified: true,
      notes:
        'MEASURED 2026-08-30 on omp 18.0.11 (Phase 169): a bracket pasted absolute .png path became an image attachment chip in the editor before any Return, the same shape as claude, so the strategy moved from the inherited path-text to paste-path. What the model receives was not seen, because no provider on the probe machine accepted a turn.'
    },
    unverified: false
  },
  {
