...
Scene: a developer at a dark desk at 11pm, six agents running, terminals filling the screen. gmux is **dark by default** and was dark-only until Phase 213 — a light chrome around dark terminals creates blinding contrast wells, and the terminal IS the page. That sentence still decides the shape of the light mode: when the scheme is light the TERMINAL GOES LIGHT TOO, and a light frame around a dark terminal is refused. The world is a quiet, slightly cool graphite with **two grounds** (Phase 284). The WORK, being the terminal, the editor beside it and the band above them, is one material: `--bg-canvas` is the work's surface AND xterm's background, identical, so inside the work the app disappears into the work. Around it sits the SURROUND, being the titlebar, the activity bar, the sidebar and the session list, on `--bg-sidebar`, one rung DARKER than the work on both bases (relative luminance 0.00482 under 0.00701 on dark, 0.86209 under 0.92836 on light). That relation is why the refusal above still holds: the frame is always the darker ground, so a light frame around a dark terminal cannot occur. The work is set into the surround inside the window's ONE complete outline, 1px of `--border-strong` on a 14px radius with an 8px gutter around it, and the surround draws no line on the side facing it (§1.9, §2.2). Color is spent on exactly one thing: **state**. An amber dot that says "needs you" is the loudest object in the interface; everything else is neutral, dense, and native. Brand lives in precision — the restore moment, the status language, the exact weight of a session row — not in decoration. Token names are theme-neutral (`--bg-canvas`, not `--gray-900`) so a light theme can be added later without renaming.

**Reversed on 2026-09-17 (Phase 284, the quiet surround).** Until that day this paragraph made the terminal canvas and the app chrome *the same* material, and §1.9 and §2.2 separated every region with a hairline. The operator chose the quiet surround from a design study of his own (`design/`, option B, `design/prototypes/index.html#surround`), in his words "i love the queit surround", and said the phase may override his earlier ruling and the kept edges research 75 records. What did NOT move: dark by default, the terminal going light with the scheme, the refusal of a light frame around a dark terminal, and colour spent on state alone. The study's own hexes did not ship. Every colour is a token that already existed (§1.1), so the treatment turns with the Appearance controls and no new neutral joined the hue lists.

## 1. Tokens

All tokens are CSS custom properties declared on `:root` in `src/renderer/styles/tokens.css`, with ONE exception, the three frame geometry tokens in `src/renderer/app/frame-geometry.css` (§1.9 says why). Components must never hardcode a color, size, duration, or font — tokens only.

### 1.1 Neutrals (cool graphite ramp, hue ≈ 222°, low sat)

```css
--bg-canvas:   #131417;  /* window base, the work's surface AND xterm background — one material */
--bg-sidebar:  #0E0F13;  /* the surround: sidebar, tab bar, the gutters round the work (below the canvas) */
...
--border:        #25282E;  /* hairlines INSIDE a region (seams), 1px always  */
--border-active: #2D3038;  /* the hairline where it sits ON --bg-active (1.105:1 there; research 75 C4) */
--border-strong: #353943;  /* input borders, resize handles on hover, the work's ONE outline */
...
**The frame's geometry (Phase 284) lives in a file of its own**, `src/renderer/app/frame-geometry.css`, which `work-area.css` pulls in with `@import`:

```css
--r-frame: 14px;    /* the work's one outline, and nothing else            */
--frame-gap: 8px;   /* the gutter between the work and the surround        */
--frame-edge: 1px;  /* the outline's width; the children clip at 14 - 1    */
```

These three are the one exception to "all tokens in tokens.css", for two measured reasons. `conformance:hue` rule 25 hashes the first `:root` block of `tokens.css` byte for byte, so a declaration added there re-pins the dark base, and the file is on that gate's path list, so touching it at all owes the 13 minute run. And they are GEOMETRY rather than appearance: each mirrors a constant in `src/renderer/state/chrome-geometry.ts` by value (`FRAME_RADIUS`, `FRAME_GAP`, `FRAME_EDGE`), and `chrome-geometry.test.ts` holds the stylesheet and the width model to one number. `--r-lg` stays reserved for modals and overlays; `--r-frame` has exactly one user. There is no 7px token for the study's row and chip: a row keeps `--r-md`, and `--r-pill` is named for a badge's shape and is not borrowed.

Shadows always carry offset + blur (no zero-offset halos). Hairline borders are 1px `--border`. **Regions are separated by ground and gutter, not by hairlines and never by shadows** (Phase 284; until 2026-09-17 this sentence read "regions are separated by hairlines", and the operator reversed it): the work sits inside ONE 1px `--border-strong` outline with an 8px gutter of `--bg-sidebar` around it, and the titlebar, the activity bar, the sidebar and the session list keep their 1px border on the side facing the work and draw it `transparent`, so no box moved when the lines went. The outline is an OVERLAY, a `::after` with `pointer-events: none` drawn 1px outside the work's box, never a border or an inset shadow, because xterm's canvas and the header backgrounds paint over an inset shadow (Phase 40's precedent) and a border changes the box xterm measures. A hairline remains where it is a seam INSIDE a region: split dividers, the editor's left edge, the band's rule, the project rail's right edge, the usage seam. A border drawn ON a selected, pressed or hovered `--bg-active` fill is `--border-active`, because `--border` reads 1.013:1 there and does not exist (research 75 C4, Phase 197).
...
│ 48· Explorer   ·8╰──────────────────────────────────┴──────────────╯8· max 320     │
└──────────────────────── 8px of the surround under the work ────────────────────────┘
                                          status strip: none — no bottom bar in v1
```

`╭ ╮ ╰ ╯` is the ONE outline in the window (Phase 284): 1px `--border-strong`, 14px radius, drawn around the work and nothing else. `·` is a boundary with no line: the pixel is still there and it is `transparent`. `8` is a gutter of `--bg-sidebar`. The work has 8px on its right ALWAYS (the gap to the session list, or the window inset when there is no list), 8px under it always, 8px on its left only while the sidebar is drawn, and 0 on top, where its line lands on the titlebar's last pixel row. There is no inset to the right of the session list and none under the sidebars: both would be the surround's colour on the surround's ground, invisible, and the first would cost every terminal another 8px. **Focus mode turns the frame OFF** (no gutter, no radius, no line), because the surround is not drawn there and a frame around nothing is the refusal in §0 turned inside out.

- **The header band (round-1 fix; its hairline moved inside the work in Phase 284).** Directly under the titlebar, one continuous 36px band crosses every region except the activity bar: the sidebar view header, the terminal-region header (session tab strip by default, identity strip in "right" orientation), the editor tabs row (32px in round 0 — now 36px), and the right session list's toolbar when present. All are exactly 36px tall on `--bg-sidebar`, which still kills the round-0 misalignment where the 36px branch header and the 28px session strip sat at two different heights. **The 1px `--border` bottom hairline is drawn only INSIDE the work**: it runs unbroken from the work's left edge to its right edge, meets the outline at both ends in a T (it sits at y = 35 and the curve is 13px tall, so the curve never reaches it), and has exactly one sanctioned interruption, the active session tab's melt. It still turns `--accent` while a terminal has the keyboard. The band is the first 36px INSIDE the curve in both orientations, the way a window's title area is inside the window, and the first tab takes the curve's inner radius on its own top-left corner so its accent bar follows the curve instead of being cut by it. The sidebar's view header (Source Control's branch header is that view's header and goes with it, so the sidebar's head does not change as a person switches views), the session list's toolbar, the project rail's band and the activity bar's row form keep the pixel and draw it `transparent`. *Until 2026-09-17 this bullet required ONE unbroken hairline from the sidebar's left edge to the window's right edge with the region dividers crossing it. The operator reversed that with the quiet surround, and the band's rule inside the work is what is left of it.*
- **Activity bar (decision reversed from round 0).** A 48px VS Code-style icon rail at the far left, full height below the titlebar: **Explorer** (⌘⇧E) and **Source Control** (⌃⇧G) views, Settings gear at the bottom, a soft `--bg-active` chip marking the active view (Phase 284; a 2px accent bar until 2026-09-17, which the operator's quiet surround traded for the project tab's own shape). Round 0 rejected segmenting because it would hide the sessions' NEEDS_INPUT state behind a mode switch; sessions no longer live in the sidebar (next bullet), so that objection is dissolved and the sidebar adopts the single-view VS Code pattern — earned familiarity, and full sidebar height for both SCM and the tree.
...
- **Terminal is the center.** The region shows one *surface* per project at a time: a surface is one session filling the framed work or — round 2 — a drag-built **split group of up to 6 sessions** (DESIGN-SPEC S4A; reverses round 1's "no splits"). Every split stays its own durable tmux-backed session — layout is app-side presentation only. Switching surfaces swaps the region (hidden sessions cost nothing — architecture). Rename lives on the tab / list row / identity strip / split header (F2 or double-click); the ⋯ session menu lives on the identity strip and on tab/row context menus.
...
- Region boundaries (Phase 284; "Region dividers: 1px `--border`" until 2026-09-17): the work's one outline and its 8px gutters, and no divider. The drag handles are invisible 5px hit areas that sit IN the gutter, centred in the 7px the line leaves free, and show `--border-strong` on hover and `--accent` on `:focus-visible`, `col-resize` cursor. Each is a real `role="separator"` tab stop that the arrow keys move, and the session list's handle gained the focus state it never had. Seams inside the work (split dividers, the editor's left edge) stay 1px `--border`.
...
| **Session list row (right dock)** | 24px: agent icon 16 · name 13 · ⎇wt chip · right: status dot 8, × 16 on hover | selected: `--bg-active` fill + 1px `--border-active` outline on `--r-md`, no marker bar (Phase 284; a 2px `--accent` left inset until 2026-09-17). The collapsed 48px rail and the project rail are full-bleed rows, so they take the fill alone, which is the project tab's treatment; hover `--bg-raised`; same status vocabulary as tabs; age + status text in tooltip; density mirrors VS Code's terminal list |
| **Activity bar item** | 48×48 hit area, codicon 24 centered; SCM item carries a dirty-count badge (accent bg, never amber) | active: icon `--text-primary` on a `--bg-active` chip, 36px inside the 48px cell on `--r-md`, no marker bar (Phase 284; a 2px `--accent` left inset bar until 2026-09-17); inactive `--text-muted`; hover `--text-secondary` on a `--bg-raised` chip; the hit area is still the whole cell; click active view = collapse sidebar (= ⌘B); tooltip carries name + shortcut |
...
| **Split pane header** | 24px bar atop each pane when a surface has ≥2 splits: agent icon 14 · name 12 · ⎇ worktree 12 · status dot 8 · × 14 on hover; the whole bar is the drag handle (`grab`/`grabbing` cursor) | focused pane: name `--text-primary` + ONE 1px `--accent-soft` box around the WHOLE pane, header included, drawn as an overlay (Phase 40 removed the 2px left inset and the hard inset ring this row still described until Phase 284 corrected it); it is the one other region-sized box the window may draw beside the work's outline, and on a bottom corner the pane shares with the frame it follows the curve at 13px, as the first session tab does; unfocused: `--text-secondary` and the body one step back, never the header; drag to strip/dock = pop out; right-click = session context menu + "Move to its own tab" |
